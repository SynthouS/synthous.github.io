...
fn main() -> Result<(), Box<dyn std::error::Error>> {
        ...
        .subcommands([
            ...
            Command::new("new")
                ...
                .args(&[
                    ...
                    arg!(<PROJECT> "Project name").value_hint(ValueHint::AnyPath),
                ]),
                
            Command::new("version")
                .about("Prints the version of SPWN")
                .visible_alias("v"),
        ])
        .arg_required_else_help(true)
        .get_matches();
    ...
    if let Some(build_cmd) = matches.subcommand_matches("build") {
        ...
        let project_name = new_cmd.value_of("PROJECT").unwrap();
        let path = PathBuf::from(project_name);

        if path.exists() {
            eprint_with_color(
                &format!("Error: project '{}' already exists", project_name),
                Color::Red,
            );
            std::process::exit(ERROR_EXIT_CODE);
        }
        ...
        print_with_color(
            &format!("Creating {} package", project_name),
            Color::Green,
        );

        fs::create_dir_all(&path)?;

        let yaml_path = path.join("pckp.yaml");
        fs::write(
            &yaml_path,
            format!(
                "%YAML 1.2\n---\nname: {}\nversion: 1.0.0\nfolders:\n  - src\n",
        ...
        if new_cmd.is_present("lib") {
            let lib_path = src_path.join("lib.spwn");
            fs::write(
                lib_path,
                "#[cache_output]\n\ngreeting = () {\n    $.print(\"Hello world\")\n}\n\nreturn {\n    greeting,\n}\n",
            )?;
            ...
            print_with_color(
                "Note: change pckp.yaml before upload to pckp",
                Color::Blue,
            );
        } else {
            let main_path = src_path.join("main.spwn");
            fs::write(
                main_path,
                "\n\n$.print(\"Hello world!\")\n\n",
            )?;
        ...
        print_with_color("Done!", Color::White);

        Ok(())
    } else if matches.subcommand_matches("version").is_some() {
        println!("SPWN version {}", VERSION);
        Ok(())
    } else {
        unreachable!()
    }
